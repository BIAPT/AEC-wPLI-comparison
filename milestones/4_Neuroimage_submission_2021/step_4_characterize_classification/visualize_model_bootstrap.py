...
for s in Steps:
    pdf = matplotlib.backends.backend_pdf.PdfPages(f"NEW_AEC_wPLI_FINAL_model_bootstrap_step_{s}.pdf")
    ...
    for graph in GRAPHS:
        ...
        last_first = pd.read_csv(f"{IN_DIR}bootstrap_Final_model_{graph}_eml5_vs_emf5_step_{s}.csv")
        resp_unres = pd.read_csv(f"{IN_DIR}bootstrap_Final_model_{graph}_resp_vs_unre_step_{s}.csv")
        ...
        acc_mean=[np.array(ind['Acc_Dist Mean'][0])*100,
             ...
             np.array(last_first['Acc_Dist Mean'][0]) * 100,
             np.array(resp_unres['Acc_Dist Mean'][0]) * 100]
        ...
        acc_sd=[np.array(ind['Acc_Dist Std'][0])*100,
             ...
             np.array(last_first['Acc_Dist Std'][0])*100,
             np.array(resp_unres['Acc_Dist Std'][0])*100]
        ...
        min=[np.array(ind['acc_interval_low'][0])*100,
             ...
             np.array(last_first['acc_interval_low'][0])*100,
             np.array(resp_unres['acc_interval_low'][0])*100]
        ...
        max=[np.array(ind['acc_interval_high'][0])*100,
             ...
             np.array(last_first['acc_interval_high'][0])*100,
             np.array(resp_unres['acc_interval_high'][0])*100]

        fig = plt.figure()
        plt.bar([0,1,2,3,4,5], list(map(add, acc_mean, acc_sd)), color='C0')
        plt.bar([0,1,2,3,4,5], acc_mean, color='C1')
        plt.bar([0,1,2,3,4,5], np.array(acc_mean)-0.5, color='C0')
        plt.bar([0,1,2,3,4,5], list(map(sub, acc_mean, acc_sd)), color='white')
        ...
        plt.errorbar((4,4),(min[4],max[4]), color='black')
        plt.errorbar((5,5),(min[5],max[5]), color='black')
